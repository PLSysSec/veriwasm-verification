lemma Test%1%: 
  assumes "seps blocks"
    ...
    shows "
           (let' (fam,n,o_sig) = Get_Instr_Sig  (Instr InstructionUnderTest (MemSize*8) addr (uint (last Test%1%_src_start)));
                  test_formula = the ((the ((the ((instr_semantics semantics) (fam))) (n))) (o_sig))
               in exec_learned_instr_testing  
                    flag_annotation test_formula (Instr InstructionUnderTest (MemSize*8) addr (uint (last Test%1%_src_start)))
