...
theory Cmpxchg_rh_rh_GenSet0
  imports Cmpxchg_Setup_GenSet0 Cmpxchg_rh_rh_IOPairs_GenSet0 "reassembly_setup.Leviathan_Setup"
begin

locale cmpxchg_rh_rh_test = execution_context + strata_rules_cmpxchg
begin
...
lemma is_manual_cmpxchg_rh_rh [simp]:
  shows "is_manual assembly semantics (Binary (IS_80486 Cmpxchg) (Reg (General  EightHigh r_1)) (Storage (Reg (General EightHigh r_2)))) = False"
  by (auto simp add: is_manual_def Let'_def simp_rules unfold_semantics)

abbreviation Instr   where "Instr      \<equiv> r_r_Instr"
abbreviation RegDstSize where "RegDstSize \<equiv> EightHigh"
abbreviation RegSrcSize where "RegSrcSize \<equiv> EightHigh"
