theory x86TSO_view_partitioned_nondeterministic2
...
locale view_partitioning = fixes
    step_d        :: "'S \<Rightarrow> 'A \<Rightarrow> 'S"
and valid_actions :: "'S \<Rightarrow> ('A set)"    
and view          :: "'S \<Rightarrow> 'T \<Rightarrow> 'V"
and dom           :: "'A \<Rightarrow> 'T" (* For a given action, which thread does it below too?*)
and NOP           :: "'A"

assumes locally_respects: "(dom a) \<noteq> u \<longrightarrow> (\<forall> s' \<in> (step_nd s) . (view s u) = (view s' u))" (* Locally Respects *)
assumes step_consistency:"((view s sc) = (view t sc))  \<longrightarrow> set_view_equivalence (step_nd s) (step_nd t) sc " (* Step Consistency *)

begin

fun step_nd :: " 'S \<Rightarrow> 'S set"
  where "step_nd s = (let as = (valid_actions s)
                      in ({s' . \<exists> a \<in> as . s'=(step_d s a)})
                     )" 

fun purged_step_nd :: " 'S \<Rightarrow> 'T \<Rightarrow> 'S set"
  where "purged_step_nd s d = (let as = (valid_actions s);
                                   as'= {a \<in> as . (dom a = d)}
                      in ({s' . \<exists> a \<in> as . s'=(step_d s a)})
                     )" 

definition set_view_equivalence :: "'S set \<Rightarrow> 'S set \<Rightarrow> 'T \<Rightarrow> bool"
  where "set_view_equivalence S S' sc  \<equiv> (\<forall> s \<in> S . \<exists> s' \<in> S' . ((view s sc) = (view s' sc))) \<and>  

                                         (\<forall> s' \<in> S' . \<exists> s \<in> S . ((view s' sc) = (view s sc)))"

fun run_sub :: " (('S set ) set) \<Rightarrow> 'S set"
  where "run_sub S = union {s . s\<in>S }"

fun run :: " 'S \<Rightarrow> nat \<Rightarrow> 'S set"
  where "run s n = run_sub {s} n"
