...
class Instruction(object):
    ...
    def __init__(self, binary, addr, size, insn_bytes, capstone_instr):
        ...
        self.capstone_operand_sizes = [ operand.size for operand in capstone_instr.operands ]
...
#       comments = ( "\t## Size:%s" % (self.size) ) + comments 
    ...
    def assembly(self, comments=False, symbolized=True):
        ...
        not_symbolized = "\t%s\t%s # Size:%s, Opcode: 0x%02x,0x%02x,0x%02x,0x%02x" % (self.mnemonic, self.op_str, self.size, self.capstone_operand_sizes, self.opcode[0], self.opcode[1], self.opcode[2], self.opcode[3])
#        not_symbolized = "\t%s\t%s # Size:%s, %s,Opcode: 0x%02x,0x%02x,0x%02x,0x%02x" % (self.mnemonic, self.op_str, self.size, self.capstone_operand_sizes, self.opcode[0], self.opcode[1], self.opcode[2], self.opcode[3])
        if not symbolized:
            asm = not_symbolized
